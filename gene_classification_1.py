...
import os,json,torch,time,sys,copy,math
...
class Train_Data(Dataset):
    ...
    def __getitem__(self, index):
        ...
        dmodel=512
        decode_input=torch.from_numpy(self.target_data.iloc[ix:ix+1000,0].values)
        ...
        target_masked=self.target_masked[ix:ix+1000]
...
class Test_Data(Dataset):
    ...
    def __getitem__(self, index):
        ...
        dmodel=512
        decode_input=torch.from_numpy(self.target_data.iloc[ix:ix+1000,1].values)
        positional_vector_decoder = torch.Tensor([[math.sin(
            math.pow((i + 1) / 1000, (j + 1) / dmodel)) if j % 2 == 0 else math.cos(
            math.pow((i + 1) / 1000, (j + 1) / dmodel)) for j in range(dmodel)] for i in range(ix, ix + 1000)])
        dic['decode_input'] = decode_input
    ...
    for epoch in range(start_epoch,num_epochs):
        ...
        for phase in ['train', 'val']:
            ...
            for i, dataset in enumerate(dataloads[phase]):
                ...
                if board and epoch==start_epoch and i==0:
                    test_input1=torch.rand_like(encode_input)
                    test_input2=torch.ones_like(decode_input)
                    test_input3=torch.ones_like(target_masked)
                    test_input4=torch.rand_like(positional_vector_decoder)
                    board.add_graph(models[0], (test_input1,test_input2,test_input3,test_input4))
                ...
                if phase=='train':
                    ...
                    output = F.nll_loss(output, decode_input.view(-1), reduction='none')
                    target_masked=target_masked.bool()
...
def main():
    ...
    window_size=1000
    window_matrix=[i for i in range(0,input_data.shape[0]-window_size,window_size//2)]
    ...
    data_loader = {'train':DataLoader(train_dataset,batch_size=4,shuffle=False,num_workers=16),'test':DataLoader(test_dataset,batch_size=4,shuffle=False,num_workers=16)}
    ...
    train_model(models,data_loader,lr_sch,board=None,use_cuda=use_cuda)
